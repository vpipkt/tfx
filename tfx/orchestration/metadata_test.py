...
class MetadataTest(tf.test.TestCase):
  ...
  def testContext(self):
    with metadata.Metadata(connection_config=self._connection_config) as m:
      ...
      context_type_pipeline = m.store.get_context_type(
          metadata._CONTEXT_TYPE_PIPELINE)
      context_type_pipeline.ClearField('id')
      self.assertProtoEquals(
          """
          name: 'pipeline'
          properties {
      ...
      context_type_run = m.store.get_context_type(
          metadata._CONTEXT_TYPE_PIPELINE_RUN)
      context_type_run.ClearField('id')
      self.assertProtoEquals(
          """
          name: 'run'
          properties {
